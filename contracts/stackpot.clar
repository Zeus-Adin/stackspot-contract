...
(define-private (pay-fees (amount uint))
    (begin
        (try! (stx-transfer-memo? (* (/ amount u100) u10) tx-sender platform-address (unwrap! (to-consensus-buff? "platform-fee") NOT_FOUND)))
        (try! (stx-transfer-memo? (* (/ amount u100) u90) tx-sender (var-get pot-owner) (unwrap! (to-consensus-buff? "pot-fee") NOT_FOUND)))
        (ok true)
    )
)
(define-private (delegate-to-pot
    ...
    (let (
        ...
        (asserts! (>= participants-stx-balance (+ amount pot-fee)) INSUFFICIENT_BALANCE)

        (asserts! (not (is-eq participant pot-treasury-address)) UNAUTHORIZED_PARTICIPANT)
        (asserts! (not (is-eq participant platform-address)) UNAUTHORIZED_PARTICIPANT)
        (asserts! (not (is-eq participant (var-get pot-owner))) UNAUTHORIZED_PARTICIPANT)

        (asserts! (<= index-participants max-participants) MAX_PARTICIPANT_REACHED)
        (asserts! (not (is-some (map-get? pot-participants-by-principal participant))) DUPLICATE_PARTICIPANT)
        ...
        (try! (stx-transfer-memo? amount participant pot-treasury-address (unwrap! (to-consensus-buff? "joined pot") NOT_FOUND)))
...
(define-public (start-jackpot)
    (let (
            (pot-id (unwrap! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.stackpot-pots get-pot-info pot-treasury-address) NOT_FOUND))
        )
...
(define-public (claim-pot-reward (pot-contract <stackpot-pot-trait>))
    (let 
        (
            ;; Get pot ID
            (pot-id (unwrap! (unwrap! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.stackpot-pots get-token-id pot-treasury-address) NOT_FOUND) NOT_FOUND))
            ;; Get 
        ...
        (try! (as-contract (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.stackpot-pots claiming-pot-reward-return-principals pot-id pot-contract)))       
...
(define-private (get-random-index (participant-count uint))
    (let (
            ...
            (vrf-random-digit (unwrap! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.stackpot-vrf get-random-uint-at-block stacks-block-height ) NOT_FOUND))
        )
        (print {
