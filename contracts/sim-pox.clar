...
(define-read-only (get-pox-rejection
    (stacker principal)
    (reward-cycle uint)
  )
  (map-get? stacking-rejectors {
    stacker: stacker,
...
(define-read-only (is-pox-active (reward-cycle uint))
  (let ((reject-votes (default-to u0
...
(define-private (next-cycle-rejection-votes)
  (default-to u0
    (get amount
      (map-get? stacking-rejection { reward-cycle: (+ u1 (current-pox-reward-cycle)) })
    ))
)
...
(define-private (check-pox-addr-version (version (buff 1)))
  (or
    (is-eq version ADDRESS_VERSION_P2PKH)
...
(define-public (stack-stx
  ...
  (let (
    ...
    (try! (add-pox-addr-to-reward-cycles pox-addr first-reward-cycle lock-period
      amount-ustx
    ))
...
(define-public (stack-aggregation-commit
  ...
  (let ((partial-stacked
    ...
    (let ((amount-ustx (get stacked-amount partial-stacked)))
      ...
      (add-pox-addr-to-ith-reward-cycle u0 {
        pox-addr: pox-addr,
...
(define-public (reject-pox)
  (let (
    ...
    ;; vote for rejection
    (map-set stacking-rejection { reward-cycle: vote-reward-cycle } { amount: (+ (next-cycle-rejection-votes) balance) })
