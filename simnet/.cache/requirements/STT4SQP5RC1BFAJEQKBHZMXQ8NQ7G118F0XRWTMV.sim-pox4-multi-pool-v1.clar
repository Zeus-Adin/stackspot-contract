...
(define-constant pox-info (unwrap-panic (contract-call? 'STT4SQP5RC1BFAJEQKBHZMXQ8NQ7G118F0XRWTMV.sim-pox-4 get-pox-info)))
...
(define-private (delegate-stx-inner (amount-ustx uint) (delegate-to principal) (until-burn-ht (optional uint)))
  (let ((result-revoke
          ...
          (contract-call? 'STT4SQP5RC1BFAJEQKBHZMXQ8NQ7G118F0XRWTMV.sim-pox-4 revoke-delegate-stx)))
    ;; Calls delegate-stx, converts any error to uint
    (match (contract-call? 'STT4SQP5RC1BFAJEQKBHZMXQ8NQ7G118F0XRWTMV.sim-pox-4 delegate-stx amount-ustx delegate-to until-burn-ht none)
      success (ok success)
...
(define-private (lock-delegated-stx (user principal))
  (let ((start-burn-ht (+ burn-block-height u1))
    ...
    (match (contract-call? 'STT4SQP5RC1BFAJEQKBHZMXQ8NQ7G118F0XRWTMV.sim-pox-4 delegate-stack-stx
             user amount-ustx
      ...
      success-extend (let ((unlock-burn-height (get unlock-burn-height success-extend)))
            (if (is-eq amount-ustx locked-amount)
                ...
                (let ((increase-by (- amount-ustx locked-amount)))
                  (match (contract-call? 'STT4SQP5RC1BFAJEQKBHZMXQ8NQ7G118F0XRWTMV.sim-pox-4 delegate-stack-increase
...
(define-private (maybe-extend-for-next-cycle
  ...
  (let ((current-cycle (contract-call? 'STT4SQP5RC1BFAJEQKBHZMXQ8NQ7G118F0XRWTMV.sim-pox-4 current-pox-reward-cycle))
        (unlock-height (get unlock-height status)))
    (if (not-locked-for-cycle unlock-height (+ u1 current-cycle))
      (contract-call? 'STT4SQP5RC1BFAJEQKBHZMXQ8NQ7G118F0XRWTMV.sim-pox-4 delegate-stack-extend
...
(define-public (stack-aggregation-increase (current-cycle uint) (index uint)
  ...
  (let ((reward-cycle (+ u1 current-cycle)))
    (as-contract (contract-call? 'STT4SQP5RC1BFAJEQKBHZMXQ8NQ7G118F0XRWTMV.sim-pox-4 stack-aggregation-increase (var-get pool-pox-address) reward-cycle index signer-sig signer-key max-amount auth-id))))
...
(define-public (stack-aggregation-commit (current-cycle uint)
                  ...
                  (max-amount uint) (auth-id uint))
  (let ((reward-cycle (+ u1 current-cycle)))
    (as-contract (contract-call? 'STT4SQP5RC1BFAJEQKBHZMXQ8NQ7G118F0XRWTMV.sim-pox-4 stack-aggregation-commit-indexed (var-get pool-pox-address) reward-cycle signer-sig signer-key max-amount auth-id))))
...
(define-read-only (get-delegated-amount (user principal))
  (default-to u0 (get amount-ustx (contract-call? 'STT4SQP5RC1BFAJEQKBHZMXQ8NQ7G118F0XRWTMV.sim-pox-4 get-delegation-info user))))
